...
def _print_banner():
    ...
    if not config.WEBHOOK_URL:
        ...
        tips.append("   当前使用 Webhook 模式，通过 CF 隧道暴露服务", style="dim white")
...
async def _send_audio_with_fallback(context, chat_id, song, quality="standard", caption=None, 
    ...
    # 2. 下载到本地临时文件，写入ID3标签和封面，发送后删除
    import tempfile
    temp_file_path = None
    try:
        logger.info(f"{log_prefix}⬇️ 下载到本地: {song['name']} - {song['artist']}")
        ...
        try:
            ...
            return True, msg.audio.file_id if msg and msg.audio else None, "本地下载"
    ...
    async def _send_all():
        ...
        for idx, song in enumerate(songs, 1):
            ...
            try:
                ...
                # 歌单播放：下载到本地 → 写入ID3 → 发送 → 删除临时文件
                import tempfile
                temp_file_path = None
                sent = False
                try:
                    logger.info(f"歌单播放 ⬇️ 下载到本地: {song['name']} - {song['artist']}")
...
async def _resume_playlist_play(application, user_id: int, playlist_id: int, songs: list, start_index: int, total: int):
    ...
    for idx_offset, song in enumerate(songs):
        ...
        try:
            ...
            sent = False
            try:
                logger.info(f"歌单续播 ⬇️ 下载到本地: {song['name']} - {song['artist']}")
                direct_url = await asyncio.to_thread(api.get_first_song_url, song["id"], db.get_quality())
                if not direct_url:
...
async def _play_song(update: Update, context: ContextTypes.DEFAULT_TYPE, song_id: int, edit: bool = False):
    ...
    # 下载到本地临时文件 → 写入ID3 → 发送 → 删除临时文件
    import tempfile
    temp_file_path = None
    logger.info(f"播放歌曲 ⬇️ 下载到本地: {song['name']} - {song['artist']} (用户={user_label})")
    if not url:
...
async def handle_inline_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for song in valid_songs:
        ...
        if cached_fid and str(cached_fid).strip():
            ...
            # 未缓存：使用本地音频代理端点（从网易云下载+写入ID3标签和封面）
            cover_param = ""
            ...
            logger.info(f"内联结果 本地代理 {song['name']} proxy_url长度={len(proxy_url)}")
            results.append(
                InlineQueryResultAudio(
                    id=f"url_{song['id']}",
                    audio_url=proxy_url,
...
async def cmd_cachesameall(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """管理员：扫描Upstash中已缓存的歌曲，自动补全同一歌手同名歌曲的其他版本"""
    user = update.effective_user
    if not _is_admin(user.id):
        await update.message.reply_text("⛔ 权限不足，仅管理员可使用此命令。")
        return
...
def main():
    ...
    # 强制使用 Webhook 模式（本地部署版禁止使用长轮询）
    if not config.WEBHOOK_URL:
        _log_status("❌ 错误：未配置 WEBHOOK_URL，本地部署版必须使用 Webhook 模式！", "error")
        _log_status("请在 .env 文件中配置 WEBHOOK_URL（CF 隧道地址）", "warning")
        _log_status("或运行 start_cf_tunnel.bat 启动 CF 隧道后获取地址", "info")
        sys.exit(1)
